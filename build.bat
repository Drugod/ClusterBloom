...
TITLE THE ALMIGHTY CLUSTEBLOOM!
...
EXPLORER https://github.com/Drugod/ClusterBloom
ECHO.
GOTO :EOF
...
IF %featuresGit% NEQ True (
    ...
    IF "%projectName%" NEQ "ClusterBloom_proj" SET "projectName=ClusterBloom_proj"
    GOTO :EOF
)
...
REM Avoid redundancy
IF "%projectName%" NEQ "ClusterBloom_proj-%GitCommitHash%" SET "projectName=ClusterBloom_proj-%GitCommitHash%"
GOTO :EOF
